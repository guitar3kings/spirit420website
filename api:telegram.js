...
⏰ Заказ получен: ${new Date().toLocaleString('ru-RU', { timeZone: 'Asia/Bangkok' })}
    ...
    // НОВОЕ: Сохраняем заказ в БД бота через Railway API
    // Замени URL на свой от Railway после деплоя!
    const RAILWAY_API = 'https://твой-railway-api.railway.app/api/orders';
    ...
    try {
      ...
      console.log('Failed to save to Railway DB:', err);
      // Не критично, заказ всё равно отправлен в Telegram
    }
